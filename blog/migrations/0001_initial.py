# Generated by Django 4.1.1 on 2022-09-15 07:51

import django.core.validators
from django.db import migrations, models
import django.db.models.deletion
import django_quill.fields
import uuid


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('user', '0001_initial'),
    ]

    operations = [
        migrations.CreateModel(
            name='Tag',
            fields=[
                ('id', models.UUIDField(default=uuid.uuid4, editable=False, primary_key=True, serialize=False, unique=True)),
                ('created', models.DateField(auto_now=True)),
                ('label', models.CharField(max_length=15, unique=True)),
            ],
            options={
                'ordering': ['-created'],
            },
        ),
        migrations.CreateModel(
            name='Essay',
            fields=[
                ('id', models.UUIDField(default=uuid.uuid4, editable=False, primary_key=True, serialize=False, unique=True)),
                ('created', models.DateTimeField(auto_now=True)),
                ('title', models.CharField(max_length=70, unique=True)),
                ('description', django_quill.fields.QuillField()),
                ('upload_image', models.ImageField(default='surface.jpg', upload_to='')),
                ('source_link', models.CharField(blank=True, max_length=2048, null=True)),
                ('demo_link', models.CharField(blank=True, max_length=2048, null=True)),
                ('total_vote', models.IntegerField(default=0)),
                ('positive_vote', models.IntegerField(default=0, validators=[django.core.validators.MaxValueValidator(100)])),
                ('slug', models.SlugField(allow_unicode=True, max_length=70, unique=True)),
                ('owner', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='user.profile')),
                ('tag', models.ManyToManyField(related_name='+', to='blog.tag')),
            ],
            options={
                'ordering': ['-created'],
            },
        ),
        migrations.CreateModel(
            name='Comment',
            fields=[
                ('id', models.UUIDField(default=uuid.uuid4, editable=False, primary_key=True, serialize=False, unique=True)),
                ('created', models.DateField(auto_now=True)),
                ('body', models.TextField(max_length=300)),
                ('value', models.CharField(choices=[('up', 'up vote'), ('down', 'down vote')], default='up', max_length=10)),
                ('essay', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='blog.essay')),
                ('owner', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.CASCADE, to='user.profile')),
            ],
            options={
                'ordering': ['-created'],
                'unique_together': {('owner', 'essay')},
            },
        ),
    ]
